    ...
    class SuffixTree {
        ...
        void printLeaves(int x) {
            if (nodes[x].next.size() == 0)
            	System.out.println("\tnode"+x+" [label=\"\",shape=point]");
            else {
        ...
        void printInternalNodes(int x) {
            if (x != root && nodes[x].next.size() > 0)
            	System.out.println("\tnode"+x+" [label=\"\",style=filled,fillcolor=lightgrey,shape=circle,width=.07,height=.07]");
        ...
        void printSLinks(int x) {
            if (nodes[x].link > 0)
            	System.out.println("\tnode"+x+" -> node"+nodes[x].link+" [label=\"\",weight=1,style=dotted]");
            for (int child : nodes[x].next.values())
                printSLinks(child);
        }
    }
